...
def ctc_beam_search_decoding_batch(probs_seqs,
                                   vocabulary,
                                   beam_size,
                                   num_processes,
                                   cutoff_prob=1.0,
    ...
    :param probs_seqs: 3-D列表，每个元素作为ctc_beam_search_decoder()使用的2-D概率列表的实例
    :type probs_seqs: 3-D list
    :param vocabulary: 词汇列表
    :type vocabulary: list
    :param beam_size: 集束搜索宽度
    :type beam_size: int
    ...
    :rtype: list
    """
    probs_seqs = [probs_seq.tolist() for probs_seq in probs_seqs]

    batch_beam_results = paddlespeech_ctcdecoders.ctc_beam_search_decoding_batch(
        probs_seqs, vocabulary, beam_size, num_processes, cutoff_prob,
